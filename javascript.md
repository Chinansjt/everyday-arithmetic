...
1. 用户输入URL，浏览器会根据用户输入的信息判断是搜索还是网址，如果是搜索内容，就将搜索内容+默认搜索引擎合成新的URL；如果用户输入的内容符合URL规则，浏览器就会根据URL协议，在这段内容上加上网络协议合成合法的URL
...
5. 网络进程首先查找本地的DNS缓存服务，如果先前已经访问过并且缓存有这个DNS数据，就会直接返回缓存数据，否则，发起请求获取DNS信息，根据DNS信息将域名解析出IP和端口号，如果没有端口号，http默认80，https默认443。如果是https请求，还需要建立TLS连接。
6. Chrome 有个机制，同一个域名同时最多只能建立 6 个TCP 连接，如果在同一个域名下同时有 10 个请求发生，那么其中 4 个请求会进入排队等待状态，直到进行中的请求完成。如果当前请求数量少于6个，会直接建立TCP连接。
7. 在TCP三次握手建立连接后，http请求加上TCP头部——包括源端口号、目的程序端口号和用于校验数据完整性的序号等，向下传输
8. 网络层在数据包上加上IP头部——包括源IP地址和目的IP地址，继续向下传输到底层
9. 在最底层物理链路层通过物理网络传输给目的服务器主机
10. 目的服务器主机网络层接收到数据包，解析出IP头部，识别出数据部分，将解开的数据包向上传输到传输层
11. 传输层获取到数据包，解析出TCP头部，识别端口，将解开的数据包进行验证或排序，得到完整数据后向上传输到应用层
12. 应用层HTTP解析请求头和请求体，如果需要重定向，HTTP直接返回HTTP响应数据的状态码301或者302，同时在请求头的`Location`字段中附上重定向地址，浏览器会根据状态码和Location进行重定向操作，于是从新走一步请求的过程；如果不是重定向，首先服务器会根据请求头中是否有关于缓存的字段，如`If-None-Match`的值来判断请求的资源是否已经更新了，如果没有更新，就返回304状态码，相当于告诉浏览器之前的缓存还可以使用，就不返回新数据了；否则，返回新数据，状态码为200，并且如果想要浏览器缓存数据的话，就在响应头中加入字段：
`Cache-Control:Max-age=2000`响应数据又顺着目的服务器的应用层—>传输层—>网络层到发起请求端的网络层—>传输层—>应用层的顺序返回网络进程.
13.  数据传输完成，TCP四次挥手断开连接。如果，浏览器或者服务器在HTTP头部加上`Connection:Keep-Alive`，TCP就一直保持连接。保持TCP连接可以省下下次需要建立连接的时间，提升资源加载速度，这个只在1.1以上版本支持
...
15. 浏览器进程收网络进程的通知后，根据当前请求页面是否是和当前tab页面是同一个站点（根域名和协议一样就被认为是同一个站点），如果是，就复用之前网页的渲染进程，否则，新创建一个单独的渲染进程
16. 浏览器主进程会发出“提交文档”的消息给对应的渲染进程，渲染进程收到消息后，会和网络进程建立传输数据的“管道”，文档数据从网络进程传输到渲染进程后，渲染进程会返回“确认提交”的消息给浏览器进程
17. 浏览器进程收到“确认提交”的消息后，会更新浏览器的页面状态，包括了安全状态、地址栏的 URL、前进后退的历史状态，并更新web页面，此时的web页面是空白页
18. 渲染进程对文档进行页面解析和子资源加载，通过HTML解析器将HTML转为浏览器可以理解的DOM树。与此同时并行的计算CSS样式，生成`styleSheets`，生成styleSheets后转化样式表里的属性值，使其标准化，如颜色属性同一转为rgb的值，不同格式的font-size转化成统一的单位，然后计算DOM树中每一个节点的样式，根据CSS继承和层叠的规则，算出DOM树中每一个节点的样式，生成ComputedStyle
19. 渲染进程生成了DOM树和ComputedStyle后，还不能直接渲染，因为还没有算出可见元素在屏幕的几何位置，因此进入布局阶段。渲染进程根据DOM树和ComputedStyle生成一棵布局树，布局树不包含不需要显示的节点，并且计算每一个可见元素的具体位置，最后保存回布局树中。
20. 得到布局树后，渲染引擎还需要为特定的节点生成专用的图层（根据z-index或需要裁剪的元素会被分为一个单独的层，并不是布局树的每个节点都包含一个图层，如果一个节点没有对应的层，那么这个节点就从属于父节点的图层），并生成一棵对应的图层树，因为浏览器的页面实际上被分成了很多图层，这些图层叠加后合成了最终的页面。
21. 生成图层为每个图层生成绘制列表，并将其提交到合成线程。
22. 合成线程将图层分成图块，并在光栅化线程池中将图块转换成位图。因为如果在一个很长的页面中，将所有的图层都显示出来是很浪费性能并且是没有必要的，所以只需要将可视区域的内容显示出来即可，这就是分块的原因。
23. 合成线程发送绘制图块命令 DrawQuad 给浏览器进程。
24. 浏览器进程根据 DrawQuad 消息生成页面，并显示到显示器上
...
首先，TCP 是位于 OSI 模型的传输层的协议。它是一种面向连接的、可靠的协议，负责在网络中建立稳定的连接通道。TCP 的核心功能包括确保数据包的顺序传输、检测数据包的丢失并进行重传、以及通过流量控制和拥塞避免机制维护网络的稳定性。简言之，TCP 负责在网络中安全、可靠地传输数据。

相对地，HTTP 是位于应用层的协议，它建立在像 TCP 这样的传输层协议之上。HTTP 定义了如何格式化和传输在网络应用（如Web浏览器和服务器）之间交换的数据。这包括了构建请求（如GET、POST请求），以及定义响应结构。HTTP 是一种无状态协议，意味着每个请求都是独立的，服务器不会保存前后请求之间的状态。

在这两者的合作中，TCP 为 HTTP 提供了一个可靠的通信通道。当一个 HTTP 请求被发起时，它首先被封装进 TCP 数据包中，然后通过 TCP 建立的连接发送。在接收端，TCP 负责确保所有数据包的顺序正确且完整到达，之后 HTTP 协议处理并解释这些数据。

因此，虽然 HTTP 和 TCP 在功能上各自独立，但它们共同协作，为网络通信提供了必要的基础。没有 TCP 的可靠性和顺序保证，HTTP 无法有效地在网络上传输数据。而没有 HTTP，我们将失去在网络上高效交换和表示复杂数据结构（如网页）的能力。