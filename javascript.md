...
HMR 依赖于 Webpack 的增量打包，而打包效率会直接影响到 HMR 的更新速度。通过优化打包速度，尤其是增量构建的速度，可以有效提高 HMR 的响应速度。
...
## 19. 提升webpack打包速度
1. 识别问题：使用 webpack-bundle-analyzer、speed-measure-webpack-plugin 等工具来监控和分析打包过程，找出性能瓶颈。
2. 优化措施： 通过报告，我发现了几个主要的优化点，并进行了以下改进：
   - 使用持久化缓存
   - 减少文件解析范围：使用include和exclude
   - 使用Dllplugin：对于项目中依赖较多的第三方库，我使用了 Webpack 的 DLLPlugin，将不常更改的第三方库单独打包，从而避免每次重新构建时重复打包这些依赖。
   - 开启多线程打包：使用 thread-loader 来并行处理繁重的 loader 任务。
   - 代码分割
   - source Map：配置合理的sourceMap选项，避免生成复杂的sourceMap文件。
3. 优化结果：通过这些优化，打包速度提升了约 50%，从原来的 60 秒降低到了 30 秒。在增量构建时，速度提升更加明显，减少了开发过程中不必要的等待时间，大幅提高了开发效率。
...
**拥塞避免的优点**
1. 减少网络拥塞：通过慢启动和线性增长的方式，TCP 可以逐步探测网络的带宽，并在拥塞发生时及时减慢传输速率。
2. 提高网络稳定性：拥塞控制机制确保数据传输不会因网络拥塞而长期中断。
...
Monorepo（单一代码库）是一种将多个项目或模块存储在同一个代码仓库中的方法，而不是将每个项目分开存放在不同的仓库里。通过 Monorepo，可以在一个仓库中管理多个相互依赖的项目或模块，常用于管理大型项目、共享代码库或多个互相关联的包。

1. 单一仓库：多个项目或模块共享一个仓库，可以在同一个仓库中管理前端、后端、库、工具等多个子项目。
2. 统一依赖管理：在 Monorepo 中，通常使用统一的工具（如 npm、Yarn）管理所有模块的依赖，方便协调版本、处理依赖冲突。
3. 模块间共享：多个项目或模块可以轻松共享代码，不需要像在多仓库中那样进行发布和安装。
...
## 跨端兼容性
1. UI 组件差异（不同平台对UI组件的样式和行为处理不同，可能导致界面显示不一致）
  - 解决方法：重置默认样式，尽可能使用flex布局，使用跨平台组件库
2. JS事件处理（不同平台可能对触摸、点击等事件的支持存在差异）
  - 解决办法：将事件处理逻辑封装成函数，根据平台执行不同的逻辑
3. API 兼容性（某些功能在某个平台上可用而在另一个平台上不可用）
  - 解决办法：使用条件编译，根据当前平台使用不同的API
4. 性能差异
  - 解决办法：尽可能的优化性能
5. 存储和缓存（不同平台对本地存储的支持和实现方式可能有所不同）
  - 解决办法：抽象出统一的存储接口，底层根据平台实现具体的存储逻辑
6. 用户行为差异（不同平台的用户习惯和交互方式可能有所不同，影响用户体验）
  - 解决办法：了解目标用户在各个平台上的行为习惯，进行有针对性的设计和优化。
...
## 自我介绍
面试官您好，我叫，目前拥有5年的前端开发工作经验，在电商、外卖和直播行业有一定的涉猎。
我的核心技术栈为Vue，有微信小程序和uni-app、taro跨平台应用开发的实战经验。我对前端工程化
和性能优化有一定的理解和实践，比如之前在一个大型外卖平台项目中，我通过切片和懒加载策略，显著提升了外卖点餐页面的响应速度和整体性能，提高了用户体验。
...
## 如何推动方案落地
1. 判断当前的这个方案对业务能产生多少价值，当收获的价值远超于付出的成本时。可以选择
2. 自己花时间专研这个方案，如何使用，并且可以让团队以最低的成本熟悉这个方案。
3. 编写使用文档，列出可能会出现的问题，并且给出解决办法，如果同事在使用的过程中出现问题，要确定自己能够快速解决。
4. 如果有必要可以进行简单的培训，但是要控制在最短的时间内，避免占用过多的时间成本。

## 项目进度管控
1. 明确优先级
   - 跟产品和团队成员讨论，明确我们的核心目标和最重要的功能需求。列出核心目标后对目标进行拆解
   - 将大任务拆解成一个个小的可执行的任务，并且明确任务的优先级，确保关键任务可以按时交付。
2. 合理分配资源，明确职责
   - 充分了解团队的每一位同学，根据他们擅长的领域分配任务，将任务落实到每个人，确保他们清楚自己的职责。
3. 快速迭代和快速反馈
   - 应对比较紧急的需求时，采用快速迭代策略，当每个模块的完成时，就进入测试阶段，快速的发现问题并修改问题。
   - 及时沟通和调整，避免最最后发现重点的问题难以调整。
4. 需求变更
   - 需求变更是常有的事情，当出现需求变更时，快速评估是否会对当前进度产生影响，确保变更不会影响整体的交付时间。
5. 跟踪进度
   - 项目进展过程中，定期开短会来讨论大家的任务进度，有没有遇到棘手的问题，需不需要帮助，做到及时发现问题和解决问题。
   - 确定任务里程碑，在指定的时间内需要到达什么样的进度。

## 加密算法
前端涉及的加密算法有对称加密、非对称加密和哈希算法
1. 对称加密：对称加密指的是加密和解密都使用一个密钥来进行
   - AES
   - DES
2. 非对称加密：加密和解密都各自有一个密钥
   - RSA：https加密算法
   - ECC(椭圆曲线加密)：tls加密算法
3. 哈希算法：将任意长度的数据转换为固定长度的字符串（散列值）
   - MD5
   - SHA
   - PBKDF2
...
## 提升页面性能
1. 升级HTTP2.0
  - 升级http2.0，前端不需要配置。只要走https连接就会默认发起2.0版
  - 在Nginx上配置nginx.conf文件配置支持http2.0，在serve函数上添加`listen 443 ssl http2;`
2. 使用 HTML5 的 web workers
  - 将复杂的价格计算开启多线程来计算，避免它阻塞主线程。
  - 把请求优惠卷等数据可以放在workers上执行。
  - 建立一个新文件，在点餐页面创建一个 `new workers`，将用户选择了的商品通过`postMessage`传递给workers，由它计算完成后，再传会给主线程。点餐页面监听onmessage事件。
3. 统一图片格式为webp格式
  - 将商品的图片转为webp格式，webp图片体积更小，而且还能保持更高的分辨率。
  - 在前端中可以使用`picture`标签定义不同的源，浏览器会默认选择优先支持的。如果不支持picture标签可以在标签内定义`img`标签回退。css使用`background-image: image-set（）`
  - 在图片url请求中，服务器会根据请求头`accept`字段判断浏览器支持哪些图片格式，然后返回对应的图片格式。