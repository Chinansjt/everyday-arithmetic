...
当执行环境执行同步代码时，这些代码在调用栈中运行。在执行的过程中如果发现有异步的任务，他就会执行异步任务然后把回调放在对于的事件队列里。一旦调用栈为空，事件循环会检查消息队列。如果存在宏任务，它会执行一个宏任务，在执行宏任务的过程中，如果遇到微任务，会把微任务添加到微任务队列，在执行完当前宏任务之后并不会马上执行下一个宏任务，而是执行微任务队列的任务。完成这些后，浏览器可能会进行UI渲染。然后事件循环将移至下一个宏任务，并重复此过程。
...
10天复习计划
前3天要把复习计划表的所有知识点掌握，由浅到深，先了解这个知识点再深入研究原理实现
3天基础知识要求看到这个知识点就可以随口答上，不要有迟疑

用2天来研究重点知识点原理实现，深挖。如vue的响应式原理、webpack的hrm、构建打包原理等

最后5天根据简历围绕复习，重点从uniapp、前端基建等这些方面入手

每日复习计划
上午复习前端基础 html、css、js
下午http、tcp、浏览器、webpack、vite
晚上重点是vue以及一些边界知识
